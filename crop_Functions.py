...
def rename(folder_number):
    if folder_number == 1:
        ...
        count = 1
        for file_name in os.listdir(folder_path_G_Wizard):
            source = folder_path_G_Wizard + file_name
            destination = folder_path_G_Wizard + "image" + str(count) + ".png"
            os.rename(source, destination)
            count += 1
...
def crop_Enka_G_Wizard(image, number):
    file_paths_strings = ["character", "lvl", "nick", "uid",
                          "bow_name", "artifact", "refinement", "bow_lvl",
                          "fr_lvl", "aa_lvl", "e_lvl", "q_lvl",
                          "hp_stat", "atk_stat", "def_stat", "em_stat",
    ...
    def detect_text():
        try:
            ...
            print("imageError" + str(number))
            old_file_path = folder_path_G_Wizard + "image" + str(number) + extension_string_png
            new_file_path = folder_path_errors + "image" + str(number) + extension_string_png
            shutil.move(old_file_path, new_file_path)
