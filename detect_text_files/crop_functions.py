...
def combine_crop_and_and_rename_functions():
    ...
    rename_files(folder_path_enka1)
    big_iterator = crop_and_detect_images_enka1(big_iterator)

    rename_files(folder_path_enka2)
    big_iterator = crop_and_detect_images_enka2(big_iterator)
...
def crop_and_detect_images_enka1(big_iterator):
    for root, dirs, file_names in os.walk(folder_path_enka1):
        file_names = natsorted(file_names)
        small_iterator = 1
        for file_name in file_names:
            image = cv2.imread(folder_path_enka1 + file_name, 0)
            dim = (1924, 802)
            resized = cv2.resize(image, dim)
            cropped_images = crop_image_enka1(resized, big_iterator)
            detect_text(cropped_images, folder_path_enka1, small_iterator, big_iterator)
            big_iterator = big_iterator + 1
            small_iterator = small_iterator + 1
    return big_iterator


def crop_and_detect_images_enka2(big_iterator):
    for root, dirs, file_names in os.walk(folder_path_enka2):
        file_names = natsorted(file_names)
        small_iterator = 1
        for file_name in file_names:
            image = cv2.imread(folder_path_enka2 + file_name, 0)
            dim = (1924, 802)
            resized = cv2.resize(image, dim)
            cropped_images = crop_image_enka2(resized, big_iterator)
            detect_text(cropped_images, folder_path_enka2, small_iterator, big_iterator)
            big_iterator = big_iterator + 1
            small_iterator = small_iterator + 1
    return big_iterator
...
def crop_image_enka1(image, big_iterator):
    output_images_filepaths = create_output_images_filepaths(big_iterator)
    cropped_images = crop_images(image, output_images_filepaths, crop_values_enka1)
    save_images(output_images_filepaths, cropped_images)
    return cropped_images


def crop_image_enka2(image, big_iterator):
    output_images_filepaths = create_output_images_filepaths(big_iterator)
    cropped_images = crop_images(image, output_images_filepaths, crop_values_enka2)
    save_images(output_images_filepaths, cropped_images)
    return cropped_images
